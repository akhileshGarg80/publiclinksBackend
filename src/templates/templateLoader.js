...
/**
 * Gets a single template definition by templateId
 * @param {string} templateId 
 * @returns {Object|null}
 */
export function getTemplateDefinitionById(templateId) {
  if (!templateId) return null;
  const list = getAllTemplateDefinitions();
  return list.find(t => t.templateId.toLowerCase() === String(templateId).toLowerCase()) || null;
}

/**
 * Validates whether templateId exists
 * @param {string} templateId 
 * @returns {boolean}
 */
